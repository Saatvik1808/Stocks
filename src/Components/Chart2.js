...
const Chart2 = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=RELIANCE.BSE&outputsize=compact&apikey=${API_KEY}`
      )
      .then((res) => {
        console.log(res);
        setStock(res.data);
  ...
  const data = {
    ...
    datasets: [
      {
        label: "Reliance Industries Limited (India - BSE)",
        data: x,
        fill: true,
